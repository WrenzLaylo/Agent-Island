...
import type { AgentId, ApprovalRequest, RiskLevel } from '../../shared/contracts'
...
export interface HermesApprovalDetection {
  ...
  responseKeys: Partial<Record<'once' | 'session' | 'always' | 'deny', string>>
}
...
export function resolveHermesResponseKeys(
  ...
  if (decision === 'approve') {
    const keys = state.responseKeys.once
    if (!keys) return { ok: false, reason: 'Allow once option not available' }
    return { ok: true, keys }
  }
  const keys = state.responseKeys.deny
  if (!keys) return { ok: false, reason: 'Deny option not available' }
  return { ok: true, keys }
}
